...
do
    ...
    DistanceSlider = SwatSection:AddSlider("Distance", 1, 40, 8, {flag = "Swat_Distance"}, function() 
        getgenv().Variables.SwatDistance = DistanceSlider.Value
    end) 
    local place = SwatSection:AddBind("Toggle key", Enum.KeyCode.Y, {flag = "Swat_ToggleKey"}, function() 
        LastChanged = tick()
    end)
    ...
    Variables.UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
        if not gameProcessedEvent then
            if input.KeyCode == place.Bind and tick() - LastChanged >= .3 then
                place0:Set(not Variables.CatchingTab.Flags["Swat_Enabled"], false)
            end
        end
    end)
end
...
do
    game:GetService("RunService").RenderStepped:Connect(function()
        if Variables.CatchingTab.Flags["Swat_Enabled"] then
            Mags:Activate()
        end
    end)
end
